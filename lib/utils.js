 ...
 * {b:2,a:1} => a=1&b=2
 */
Utils.objKeyAscStr = function (obj) {
  // 转换为数组
  let tempArray = [];
  for (let item in obj) {
    tempArray.push(item)
 ...
 */
Utils.formatDataHttpRequest = params => {
  const timestamp = new Date().getTime();
  params.timestamp = timestamp;
  // image不参与进行加密,与服务端协商好
  let objKeyParams = {...params};
  if (objKeyParams.image) {
    delete objKeyParams.image
  }
  const sign = (MD5(Utils.objKeyAscStr(objKeyParams) + SIGN_KEY));
  return {
    ...params,
    sign,
    timestamp: timestamp
...
Utils.files = {
  ...
  uploadFile: function (tempFilePaths) {
    return new Promise((resolve, reject) => {
      const filePath = typeof tempFilePaths === 'string' ? tempFilePaths : tempFilePaths[0];
      wx.uploadFile({
        url: `${BASE_URL}/v1/upload/image`, //仅为示例，非真实的接口地址
        header: {
          ...Utils.httpHeader()
        },
        formData: Utils.formatDataHttpRequest({
          image: filePath
        }),
        filePath,
        name: 'image',
        ...
        fail: function (res) {
          reject(reject);
          wx.hideLoading();
          wx.showToast({title: '图片上传失败', duration: 3000});
          if (ENVIROMENT === 'developer') {
...
Utils.verificationCode = {
  ...
  action: {
    ...
    verificationCodeSuccess: function () {
      ...
      wx.showToast({title: '发送成功,请注意查收', icon: 'none', duration: 3000});
    },
    /**
     * 开始发送验证码
     */
...
Utils.signInStorage = function (data) {
  return new Promise((resolve, reject) => {
    try {
      ...
      wx.showToast({title: '存储登录信息失败!'.message, icon: 'none', duration: 3000});
      reject();
    }
  }).catch()
};
...
Utils.signOutStorage = function (data) {
  return new Promise((resolve, reject) => {
    try {
      ...
      wx.showToast({title: '删除登录信息失败!'.message, icon: 'none', duration: 3000});
      reject();
    }
  }).catch()
};
...
Utils.modal = {
  ...
  action: {
     ...
     * 延迟加载，动画结束的原因
     */
    modalOverlayToggleEndInterval: function (handle) {
...
/**
 * 页面没有内容配置选项
 * @type {{pageNoneResult: {isHidden: boolean, title: string}}}
 */
...
Utils.reachBottom = {
  ...
  action: {
    ...
    setReachEndInfo: function (info = {}) {
      ...
      // 列表填写之后延迟更新下拉加载更多的状态。
      this.setData({
        'pageNoneResult.isHidden': total !== 0,
        reachEndInfo: {
          ...reachEndInfo,
          isHidden: !(!pageNoneResult || total !== 0),
          isMore: total > listTotal && total > 15,
          isLoading: false
        }
      })
    },
...
Utils.pullDownRefresh = {
  ...
  action: {
    ...
    startPullDownRefresh: function (onBefore, onAfter) {
      ...
      wx.showNavigationBarLoading();
      const asyncDataListLength = this.data.asyncData.list.length
      this.setData({
        'refresh.isLoading': true,
        'pageNoneResult.isHidden': true,
        'searchForm.page': 1,
        'searchForm.page_size': asyncDataListLength < 10 ? 10 : asyncDataListLength
      }, () => {
        this.initFetch();
        onAfter && onAfter();
      })
  ...
  action: {
    ...
    _getFormatFetchDataParams: function ({params, data}) {
      ...
      return {
        ...
        'pageNoneResult.isHidden': data.total !== 0,
        asyncData: {
          total: data.total,
