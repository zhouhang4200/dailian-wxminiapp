 ...
 * 通知首页刷新
 */
Utils.setIsRefreshHome = function () {
  let app = getApp();
  app.globalData.isRefreshHome = true;
};
...
Utils.getUserToken = function () {
  try {
    ...
    if (token && (token && token_expired_date && DayJs(token_expired_date).diff(DayJs(), 'days') < 29)) {
      return token
    }
    else {
  ...
  action: {
    ...
    _getFormatFetchDataParams: function ({params, data}) {
      ...
      return {
        ...
        'searchForm.page_size': params.page_size,
      }
    },
    /**
