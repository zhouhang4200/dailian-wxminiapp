            ...
            api_getOpenId({code}).then(data => {
              ...
              if (!openid) {
                ...
                if (ENVIROMENT === 'developer') {
                  console.log("%c 获取openid失败,错误信息为：%c " + JSON.stringify(data), "background-color:#F56C6C;color:#fff;padding:2px 4px;font-weight:bold;font-size:12px", "color:#fff;font-size:12px");
                  console.log("%c 获取openid失败,微信code获取成功：%c " + code, "background-color:#F56C6C;color:#fff;padding:2px 4px;font-weight:bold;font-size:12px", "color:#fff;font-size:12px");
                }
              }
